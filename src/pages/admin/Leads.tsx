      ...
      {onboardingRequests.length > 0 && (
        <div className="surface-card overflow-hidden mb-5">
          <div className="px-5 py-3 border-b border-border bg-surface flex items-center gap-2">
            ...
            <div className="text-sm font-display font-semibold text-foreground">New onboarding requests</div>
            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-warning/15 text-warning">{onboardingRequests.length} new</span>
            <div className="ml-auto text-[11px] text-foreground-muted">Source: Website onboarding form</div>
          </div>
          <table className="w-full text-sm">
            <thead><tr className="text-[11px] uppercase text-foreground-muted bg-surface/50">
              ...
              <th className="text-left px-2 py-2.5">Submitted</th>
              ...
              {onboardingRequests.map((r) => (
                <tr key={r.id} className="data-row">
                  <td className="px-5 py-3">
                    <div className="font-semibold text-foreground">{r.clinicName}</div>
                    <div className="text-[11px] text-foreground-muted">{r.locations[0]?.city} · {r.locations.length} location{r.locations.length !== 1 ? "s" : ""}</div>
                  </td>
                  <td className="px-2 py-3">
                    <div className="text-foreground">{r.contactName}</div>
                    <div className="text-[11px] text-foreground-muted">{r.email}</div>
                  </td>
                  <td className="px-2 py-3 text-xs text-foreground-muted">{r.practiceType}</td>
                  <td className="px-2 py-3 text-xs text-foreground-muted">{r.channels.join(", ")}</td>
                  <td className="px-2 py-3"><StatusBadge tone={stageTone[r.status]} dot>{r.status}</StatusBadge></td>
                  <td className="px-2 py-3 text-foreground-muted">{r.submittedAt}</td>
